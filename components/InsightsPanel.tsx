...
const InsightsPanel: React.FC<InsightsPanelProps> = ({ journalEntries, insights, setInsights }) => {
  const [isLoading, setIsLoading] = useState(false);

  const handleGenerateInsights = async () => {
    setIsLoading(true);
    try {
      const newInsights = await generateInsights(journalEntries);
    ...
    <div className="flex flex-col bg-slate-800/50 rounded-lg p-6 shadow-lg h-full w-full">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          disabled={isLoading || journalEntries.length < 1}
      ...
      <p className="text-slate-400 mb-6">Insights are generated using your journal entries to help you notice patterns and reflect. You need at least one entry to begin.</p>
